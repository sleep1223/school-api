...
from school_api.client.api.plan_schedule import PlanSchedule
...
class UserClient(BaseUserClient):
    ...
    def __init__(self, school_obj, account, password, user_type):
        super().__init__(school_obj, account, password, user_type)
    ...
    def get_place_schedule(self, *args, **kwargs):
        print('test 2', args, kwargs)
        return self.place_schedule.get_schedule(*args, **kwargs)

    @error_handle
    @ApiPermissions([UserType.STUDENT])
    def get_plan_schedule(self, *args, **kwargs):
        return self.plan_schedule.get_schedule(*args, **kwargs)
